import apply_forces from '../wgsl/apply_forces.wgsl';
import create_adjacency_matrix from '../wgsl/create_adjacency_matrix.wgsl';
import create_quadtree from '../wgsl/create_quadtree.wgsl';
import create_targetlist from '../wgsl/create_targetlist.wgsl';
import create_sourcelist from '../wgsl/create_sourcelist.wgsl';
import compute_attract_forces from '../wgsl/compute_attract_forces.wgsl';
import compute_forces from '../wgsl/compute_forces.wgsl';
import compute_forcesBH from '../wgsl/compute_forcesBH.wgsl';
import compute_attractive_new from '../wgsl/compute_attractive_new.wgsl';
...
export class ForceDirected {
    ...
    async runForces(
        ...
        new Float32Array(mapping).set([this.coolingFactor, l, gravity], 2);
        uploadBuffer.unmap();

        commandEncoder = this.device.createCommandEncoder();
        commandEncoder.copyBufferToBuffer(uploadBuffer, 0, this.paramsBuffer, 0, 5 * 4);
        ...
        while (iterationCount > 0 && (this.coolingFactor > 0.0001 || true) && !signal.aborted) {
            ...
            // }
            stackBuffer.destroy();
            this.coolingFactor = Math.max(0.01, this.coolingFactor * coolingFactor);

            await positionReadBuffer.mapAsync(GPUMapMode.READ);
            let positionArrayBuffer = positionReadBuffer.getMappedRange();
            let positionList: number[] = [...new Float32Array(positionArrayBuffer)];
            await this.device.queue.onSubmittedWorkDone();
        ...
        // await positionReadBuffer.mapAsync(GPUMapMode.READ);
        // let positionArrayBuffer = positionReadBuffer.getMappedRange();
        // let positionList: number[] = new Float32Array(positionArrayBuffer) as any;
        // await this.device.queue.onSubmittedWorkDone();
        ...
        console.log(`Completed in ${numIterations} iterations with total time ${totalEnd - totalStart} and average iteration time ${iterAvg}`);
